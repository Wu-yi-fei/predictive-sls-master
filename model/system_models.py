...
class LTI_System(SystemModel):
    ...
    def systemProgress(self, u, w=None, hat_w=None):
        # x[t] -> x[t+1]
        # (y[t-1]) -> y[t] if its a output-feedback sys
        # (z[t-1]) -> z[t] if its a output-feedback sys
        if u.shape[0] != self._n_u:
            return self.errorMessage('Dimension mismatch: u')

        if w is not None:
            if w.shape[0] != self._n_w:
                return self.errorMessage('Dimension mismatch: w')
            if hat_w is not None and hat_w.shape[0] != self._n_w:
                return self.errorMessage('mismatch prediction: hat_w')
            self._x = (
    ...
    def updateAction(self, new_act_ids=None):
        '''
        develop a new system with the old dynamics and updated sub-actuator
        '''
