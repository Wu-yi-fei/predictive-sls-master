    ...
    def run(self, initialize=True):
        ...
        if initialize:
            ...
            if self._pred is not None:
                ...
                ws = self._pred.getNoiseTrajectroy1()
        ...
        for t in range(self._horizon):
            ...
            u = self._controller.getControl(y=x, t=t)
            y = self._system.getMeasurement()
            z = self._system.getOutput()
            w_history.append(self._controller._delta[0])
            x_history.append(x)
            y_history.append(y)
            z_history.append(z)
            u_history.append(u)
            ...
            # print("x(t+1)=", self._system.getState())
        return x_history, y_history, z_history, u_history, w_history

class Causal_Riccati(ObjBase):
    '''
    ...
    def run(self, A, B, Q, R, initialize=True):
        ...
        for t in range(self._horizon):
            ...
            u = self._controller.getControl(y=x)
...
class Noncausal_Riccati_Optimal(ObjBase):
    '''
    Synthesizing the predictive controller using System Level Synthesis method.
    '''
    ...
    def run(self, A, B, Q, R, initialize=True):
        ...
        for t in range(self._horizon):
            if t == 0:
                ...
                self._pred.initialize()
                ws = self._pred.getNoiseTrajectroy()
                u = self._controller.initialize(hat_ws=ws)
                self._system.systemProgress(u=u, w=w)
                x = self._system.getState()
                x_history.append(x)
                u_history.append(u)
        ...
        return x_history, u_history, w_history, ws
