        ...
        if sls._predictive:
            ...
            for t in range(len(Phi_x)):
                self._objective_expression += cp.sum_squares(
                    Q_sqrt @ Phi_x[t] @ Cov_w_sqrt +
                    Q_sqrt @ Phi_hat_x[len(Phi_x) - t] @ Cov_hat_w_sqrt) + cp.sum_squares(
                    R_sqrt @ Phi_u[t] @ Cov_w_sqrt +
                    R_sqrt @ Phi_hat_u[len(Phi_x) - t] @ Cov_hat_w_sqrt
                )
            ...
            for t in range(len(Phi_x)):
                self._objective_expression += cp.sum_squares(
                    Q_sqrt @ Phi_x[t] @ Cov_w_sqrt) + cp.sum_squares(
                    R_sqrt @ Phi_u[t] @ Cov_w_sqrt
                )
