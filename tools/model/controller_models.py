...
class SLS_StateFeedback_Controller(ControllerModel):
    ...
    def controlConvergence(self, y, **kwargs):
        ...
        u = self._conv_computation(M=self._Phi_u, N=tilde_delta, lb=1, ub=self._fir_horizon, k=1)
        return u

    def getControl(self, y, **kwargs):
        self._FIFO_insert(self._delta, y - self._tilde_x, self._fir_horizon)

        u =self._conv_computation(M=self._Phi_u, N=self._delta, lb=0, ub=self._fir_horizon, k=0)
        self._tilde_x = self._conv_computation(M=self._Phi_x, N=self._delta, lb=1, ub=self._fir_horizon+1, k=1)
...
class Predictive_SLS_StateFeedback_Controller(ControllerModel):
    ...
    def getControl(self, y, t=0):
        ...
        print("-------------t=", t)
        u = (
                self._conv_computation(M=self._Phi_u, N=self._delta, lb=0, ub=self._fir_horizon, k=0) +
             self._conv_computation(M=self._Phi_hat_u[t*self._fir_horizon:(t+1)*self._fir_horizon], N=self._hat_delta, lb=0, ub=self._fir_horizon, k=0)
        )
        self._tilde_x = (
            self._conv_computation(M=self._Phi_x, N=self._delta, lb=1, ub=self._fir_horizon, k=1) +
             self._conv_computation(M=self._Phi_hat_x[t*self._fir_horizon:(t+1)*self._fir_horizon], N=self._hat_delta, lb=0, ub=self._fir_horizon, k=0)
        )
        return u
