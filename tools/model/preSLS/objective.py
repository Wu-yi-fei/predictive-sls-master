                ...
                for k in range(len(Phi_x)):
                    if k <= t:
                        self._objective_expression += cp.sum_squares(
                            Q_sqrt @ Phi_x[k] +
                            Q_sqrt @ Phi_hat_x[t * sls._fir_horizon + t - k]) + cp.sum_squares(
                            R_sqrt @ Phi_u[k] +
                            R_sqrt @ Phi_hat_u[t * sls._fir_horizon + t - k]
                        )
                    else:
                        self._objective_expression += cp.sum_squares(
                            Q_sqrt @ Phi_hat_x[t * sls._fir_horizon + k]) + cp.sum_squares(
                            R_sqrt @ Phi_hat_u[t * sls._fir_horizon + k] # (t + 1) * sls._fir_horizon - k - 1
                        )
        else:
            # feedback
            Phi_x = sls._Phi_x
